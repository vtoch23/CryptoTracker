...
def refresh_prices(db: Session = Depends(dependencies.get_db)):
    """
    ...
    try:
        ...
        for i in range(0, len(coin_ids), batch_size):
            ...
            logger.info(f"\n{'=' * 60}")
            logger.info(f"BATCH {i//batch_size + 1}/{(len(coin_ids) + batch_size - 1)//batch_size}")
            logger.info(f"{'=' * 60}")
            ...
            try:
                ...
                logger.info(f"DEBUG: Response keys = {list(batch_data.keys())}")
                logger.info(f"DEBUG: Response length = {len(batch_data)}")
        ...
        logger.info(f"DEBUG: all_prices is empty? {len(all_prices) == 0}")
